...
pub struct TurnStreamEvent {
    ...
    pub seq: u64,
    /// The responding agent/desk this turn belongs to, so the console attaches
    /// the live steps to the correct in-flight thread.
...
/// thread: which company to publish on, and which responding agent/desk the
/// frames belong to. Built by the harness brain (which knows both) and handed to
/// the turn runner; `None` disables live streaming (e.g. background/system
/// turns and the default non-`openhuman` build).
#[derive(Clone, Debug)]
...
mod tests {
    ...
    async fn publish_reaches_subscriber() {
        ...
        publish(&company, frame("tool_call", 0).with_agent("ceo"));
