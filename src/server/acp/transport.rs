...
async fn open_session(
    state: &AppState,
    auth: &crate::server::graphql::auth::GqlAuth,
    params: &Value,
) -> Result<Value, String> {
    ...
    if state.registry().get(&company).is_none() {
        return Err(format!("company `{company}` was not found"));
    }
    ...
    Ok(
        json!({ "sessionId": id, "_meta": super::session::cwd_meta("server-side company workspace") }),
    )
}
