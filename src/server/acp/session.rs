...
//!
//! A session is the triple **(company, desk, optional agent)**. The desk is the
//! chat thread the turns land in, so an ACP client and the web console looking
//! at the same desk see the same conversation — which is the whole reason to
//! reuse the thread rather than invent a parallel one.
...
pub struct AcpSession {
    ...
    /// The desk these turns land in — the same thread the console shows.
    pub chat: String,
