...
/// The gap this section pins: `pause` stops the whole company, and it accepted
/// any signed-in member because addressing the company was mistaken for
/// authority over it.
#[tokio::test]
...
/// against a stray click, never a role check — it is published, so a member
/// knows it as well as an admin does, and `emergency-resume`'s stronger
/// confirmation is the company's own id, which a member necessarily knows.
#[tokio::test]
