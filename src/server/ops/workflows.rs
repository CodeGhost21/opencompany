...
async fn validate_workflow(
    ...
    let draft = RawWorkflow::try_from(body)?;
    // Run in the same order Create runs it, so the FIRST problem reported is the
    // one Create would report. A pre-flight that named a different problem than
    // the submit would is a worse experience than no pre-flight.
    reject_undeliverable_channel_destinations(&company, &draft)?;
    let record = company
        .runtime
        .store()
