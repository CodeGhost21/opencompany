...
pub enum OpenCompanyError {
    ...
    /// outcomes — a customer that does not exist, a payment method the site has
    /// not enabled — as 4xx responses whose JSON body names the real problem.
    /// The agent needs that body, not the status, so both are preserved; a
    /// locally-rejected argument uses `status: 0` and `code: invalid_arguments`.
