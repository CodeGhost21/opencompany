...
/// first answer exists. An empty `Artifacts/` with a README saying what will
...
pub async fn ensure_workspace_scaffold(
    ...
    let nodes = store.tree(company).await?;
    for (root, body) in [
        (SECRETS_ROOT, SECRETS_README),
        (ARTIFACTS_ROOT, ARTIFACTS_README),
    ] {
        let root_id = match find(&nodes, None, root) {
            ...
            Found::Collision(why) => {
                tracing::warn!(
                    company = %company,
                    "[workspace] {why}; not provisioning `{root}/README.md`"
                );
                continue;
            }
            Found::Free => continue,
        ...
        match find(&nodes, Some(root_id.as_str()), "README.md") {
            Found::Folder(_) | Found::Collision(_) => tracing::warn!(
                company = %company,
                "[workspace] `{root}/README.md` is not one unambiguous note; leaving it untouched"
            ),
            Found::Free => {
                ...
                if let Err(error) = store.create(company, &readme, Some(body)).await {
                    tracing::warn!(
                        company = %company,
                        %error,
                        "[workspace] could not create `{root}/README.md`; will retry on the next boot"
                    );
                }
            }
        }
