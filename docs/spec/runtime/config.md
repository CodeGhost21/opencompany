...
| `OPENCOMPANY_AUTH_MODE` | — (each company's `[users].mode`, itself defaulting to `email`) | `email` \| `wallet` \| `none` — how humans sign in, for **every** company this host serves. Set by the desktop app (always `none`) and by a platform that must guarantee a mode across tenants. An unparseable value aborts boot; `none` on a routable bind aborts boot. See [sign-in modes](auth-modes.md) |
