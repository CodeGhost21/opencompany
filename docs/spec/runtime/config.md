...
| `OPENCOMPANY_SHUTDOWN_GRACE_SECONDS` | `25` | How long a SIGTERM/SIGINT waits for in-flight turns before exiting anyway. `0` exits as soon as the companies are quiesced. Must stay below the pod's `terminationGracePeriodSeconds` — see [shutdown](lifecycle.md#shutdown) |
