...
An unrecognised focus resolves to `None`, which inherits — the pre-focus
behaviour. Worse, but never broken: a model that invents a value should cost that
teammate its narrowing, not cost the operator their roster.
