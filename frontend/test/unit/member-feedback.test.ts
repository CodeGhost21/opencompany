 ...
 *
 * Both `boot()`s swallow their own errors — `TeamView`'s goes further and swaps
 * the roster for the starter team — so before this the console could POST a
 * teammate, fail to read them back, and toast "Added Ada." over a list Ada was
 * not in. The refetch is what the toast's timing was justified by, so it has to
 * be able to withhold the confirmation it was supposed to substantiate.
 */
describe("addOutcome", () => {
