...
describe("ApprovalCard decide ordering (#1406)", () => {
  ...
  it("distinguishes two same-URL http_request cards by method (#1411)", async () => {
    ...
    expect(labelled).toContain(
      "Approve: Make a request to a web address — https://api.example.com/items — method: GET — request 1772445600000 — request ${T0} — asked by Ops",
    );
    expect(labelled).toContain(
      "Approve: Make a request to a web address — https://api.example.com/items — method: DELETE — request 1772445600000 — request ${T0} — asked by Ops",
    );
  });
  ...
  it("distinguishes two same-payload payment cards by amount (#1411)", async () => {
    ...
    expect(labelled).toContain(
      `Approve: Send a payment — ${money(40)} — recipient: acme — request ${T0} — asked by Ops`,
    );
    expect(labelled).toContain(
      `Approve: Send a payment — ${money(4000)} — recipient: acme — request ${T0} — asked by Ops`,
    );
  });
