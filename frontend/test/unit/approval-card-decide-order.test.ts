...
describe("ApprovalCard decide ordering (#1406)", () => {
  ...
  it("names each decision with the request it affects (#1411)", async () => {
    ...
    expect(labelled).toContain(
      `Approve: Run a terminal command — rm -rf /tmp/build && make release — cwd: /srv/app — request ${T0 + 60_000} — approval 1 of 1 — asked by Ops`,
    );
    expect(labelled).toContain(
      `Decline: Run a terminal command — rm -rf /tmp/build && make release — cwd: /srv/app — request ${T0 + 60_000} — approval 1 of 1 — asked by Ops`,
    );
  });
  ...
  it("distinguishes two same-URL http_request cards by method (#1411)", async () => {
    ...
    expect(labelled).toContain(
      `Approve: Make a request to a web address — https://api.example.com/items — method: GET — request ${T0 + 60_000} — approval 1 of 2 — asked by Ops`,
    );
    expect(labelled).toContain(
      `Approve: Make a request to a web address — https://api.example.com/items — method: DELETE — request ${T0 + 60_000} — approval 2 of 2 — asked by Ops`,
    );
  });
  ...
  it("keeps truncated payload labels distinct with the dropped argument (#1411)", async () => {
    ...
    expect(labelled).toContain(
      `Approve: Make a request to a web address — https://api.example.com/items — method: POST — headers: content-type: application/json — body: {"q": 1} — request ${T0 + 60_000} — approval 1 of 2 — asked by Ops`,
    );
    expect(labelled).toContain(
      `Approve: Make a request to a web address — https://api.example.com/items — method: POST — headers: content-type: application/json — body: {"q": 2} — request ${T0 + 60_000} — approval 2 of 2 — asked by Ops`,
    );
  });
  ...
  it("distinguishes two same-payload payment cards by amount (#1411)", async () => {
    ...
    expect(labelled).toContain(
      `Approve: Send a payment — ${money(40)} — recipient: acme — request ${T0 + 60_000} — approval 1 of 2 — asked by Ops`,
    );
    expect(labelled).toContain(
      `Approve: Send a payment — ${money(4000)} — recipient: acme — request ${T0 + 60_000} — approval 2 of 2 — asked by Ops`,
    );
  });
