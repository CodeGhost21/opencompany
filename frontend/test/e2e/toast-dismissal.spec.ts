...
test("a toast passes clicks on its read-only surface through to the page", async ({ page }) => {
  await settings(page);
  await replayTour(page);
  // The replay tour starts after a lazy chunk has loaded. Close that higher
  // dialog if it arrives so this test isolates the notification's hit testing.
  ...
  await page.evaluate(
    ({ x, y }) => {
      ...
      control.textContent = "Control behind toast";
      control.style.cssText = `position: fixed; left: ${x - 60}px; top: ${y - 20}px; width: 120px; height: 40px; z-index: 0;`;
      control.addEventListener("click", () => control.setAttribute("data-clicked", "true"));
      document.body.append(control);
    },
    { x, y },
  );

  // `mouse.click` is intentional: Playwright's locator click correctly refuses
  // to target an element visibly covered by a toast, while this is the native
  // pointer input an operator sends and the bug is about where it ends up.
  await page.mouse.click(x, y);
  await expect(page.getByRole("button", { name: "Control behind toast" })).toHaveAttribute(
    "data-clicked",
    "true",
  );

  // The stack's own close control remains a real control; click-through must
  // not make a toast with a close or action button inoperable.
